...
//        }
    ...
    public static String getMacAddress() {
        WifiManager manager = (WifiManager) BaseApplication.getContext().getApplicationContext()
...
//		int screenHeight = 960;
        int screenWidth = getScreenW();
        int screenHeight = getScreenH();
        Camera.Size bestSize = null;
        for (Camera.Size currentSize : sizes) {
            // boolean isDesiredRatio = Math.abs((double) currentSize.width
            // / currentSize.height)
            // - ((double) getScreenH() / getScreenW()) < 0.1;
            boolean isDesiredRatio = Math.abs(currentSize.height * screenWidth
                    - currentSize.width * screenHeight) < 1;
        ...
        Log.d("Tools", String.format("determineBestSize: %d, %d", bestSize.width, bestSize.height));
        return bestSize;
    }
